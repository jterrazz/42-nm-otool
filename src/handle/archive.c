...
/*   Updated: 2019/07/24 16:55:54 by jterrazz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "nm_otool.h"

int	parse_archive_section(t_env *env, t_file *file,
	t_ar_hdr *ar_header, uint64_t ar_size)
{
	uint64_t	name_size;
	t_file		virtual_file;
	void		*ptr;
	...
	name_size = !ft_strncmp(ar_header->ar_name,
		...
		ft_atoi(ar_header->ar_name + ft_strlen(AR_EFMT1)) : 0;
...
int	handle_archive(t_env *env, t_file *file)
{
	...
	while (ptr < file->start + file->filesize)
	{
		...
		if (check_overflow(file, ptr) || ar_size == 0
			|| check_overflow(file, ptr + ar_size))
			return (FAILURE);
		if (i++)
		{
