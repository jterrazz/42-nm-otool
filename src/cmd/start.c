...
/*   Updated: 2019/07/24 09:06:05 by jterrazz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "nm_otool.h"

static int open_file_stats(t_env *env, char const *filename, int *fd, size_t *filesize)
{
    struct stat buf;
...
int cmd_start(t_env *env, char const *filename)
{
    ...
    if (open_file_stats(env, filename, &fd, &filesize))
        return (FAILURE);
    if ((filestart = mmap(NULL, filesize, PROT_READ, MAP_PRIVATE, fd, 0)) == MAP_FAILED)
    {
